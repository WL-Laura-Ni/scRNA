...
split_obj <- SplitObject(obj,split.by = "orig.ident")
for (patient in c(paste0("P",c(1:7)))){
  split_obj[[patient]] <- NormalizeData(split_obj[[patient]],verbose=T)
  split_obj[[patient]] <- CellCycleScoring(split_obj[[patient]],
                                           s.features = cc.genes$s.genes,
                                           g2m.features = cc.genes$g2m.genes)
  split_obj[[patient]] <- SCTransform(split_obj[[patient]],
...
saveRDS(integ_obj,file = "./integ_obj_pca.rds")
...
Idents(object = integ_obj) <- "integrated_snn_res.1.8"

pdf(file = "./reduction_sct_umap_1.8.pdf",width = 8,height = 6)
